...
export default function Container() {
  ...
  return (
    <div className='w-full h-auto min-h-screen'>
      ...
      />
      <Navbar
        className={`sticky left-0 top-0 w-[100%] h-28 lg:h-20 flex flex-row flex-wrap justify-between gap-y-2 px-4 lg:px-6 pt-4 lg:pt-2 transition-opacity duration-200 ease-in-out
        ${sidebar ? "opacity-20" : "opacity-100"}`}
        setSidebar={setSidebar}
        sidebar={sidebar}
      />
      <Content
        className={`w-full h-auto pt-6transition-opacity duration-200 ease-in-out 
        ${sidebar ? "opacity-20" : "opacity-100"}`}
        setSidebar={setSidebar}
        sidebar={sidebar}
      />
